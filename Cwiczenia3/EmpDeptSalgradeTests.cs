...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        // Act - realizacja, pobranie rezultatu 
        List<Emp> result = emps.Where(e => e.Job.Equals("SALESMAN")).ToList();

        // Assert - sprawdzamy czy wynik jest poprawny 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo)
            .Select(gr => new {DeptNo = gr.Key, Count = gr.Count()})
            .ToList(); 
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo)
            .Select(gr => new {DeptNo = gr.Key, AvgSal = gr.Average(e => e.Sal)})
            .ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var avgSalary = emps.GroupBy(e =>e.DeptNo)
            .ToDictionary(gr => gr.Key, gr => gr.Average(e => e.Sal));
        var result = emps.Where(e => e.Sal > avgSalary[e.DeptNo])
            .Select(e => e.EName)
            .ToList(); 
